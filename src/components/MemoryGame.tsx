...
const MemoryGame: React.FC<MemoryGameProps> = ({ onGameComplete }) => {
  ...
  return (
    <div className="flex flex-col items-center" role="grid" aria-label="Memory game grid">
      ...
      <button
        ...
        className="mt-6 px-6 py-3 bg-green-500 text-white rounded-full shadow-lg hover:bg-green-600 transition duration-300 text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
        aria-label="Reset game, press Space or Enter"
      >
        Reset Game
      </button>
      <p className="mt-4 text-xl font-semibold text-gray-700" aria-live="polite">Moves: {moves}</p>
    </div>
  );
};
